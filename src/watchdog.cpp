...
{
    DEBUG_PRINTLN(F(CLR_YELLOW "Starting SD diagnostic..." CLR_RESET));
    
    // Retrieves the physical interface type (e.g., SDSC, SDHC, MMC)
    DEBUG_PRINTLN(F(CLR_YELLOW "Probing card hardware..." CLR_RESET));
    sdcard_type_t cardType = SD.cardType();

    // Verification: If cardType is 0 (CARD_NONE), communication failed
    if(cardType == CARD_NONE)
    {
        DEBUG_PRINTLN(F(CLR_RED "Critical Error: SD Card not detected!" CLR_RESET));
        return false;
    }

    DEBUG_PRINTLN(F(CLR_GREEN "SD Status: Healthy and Mounted." CLR_RESET));
    return true;
}